    ...
    public abstract class PartialConverter<T> : JsonConverter
        ...
        private T InternalReadJson(JsonReader reader, JsonSerializer serializer, object existingValue) {
            if (reader.TokenType != JsonToken.StartObject) {
                throw new JsonSerializationException($"Failed to read type '{typeof(T).Name}'. Expected object start, got '{reader.TokenType}' <{reader.Value}>");
            }
            ...
            while (reader.TokenType == JsonToken.PropertyName) {
                if (reader.Value is string name) {
                    if (name == previousName) {
                        throw new JsonSerializationException($"Failed to read type '{typeof(T).Name}'. Possible loop when reading property '{name}'");
                    }
